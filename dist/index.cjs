...
function inferFieldType(schema) {
	...
	if (runtimeSchema instanceof zod.z.ZodString) return { type: "string" };
	if (runtimeSchema instanceof zod.z.ZodNumber) return { type: "number" };
	if (runtimeSchema instanceof zod.z.ZodBoolean) return { type: "boolean" };
	if (runtimeSchema instanceof zod.z.ZodEnum) return {
		type: "enum",
		enumValues: runtimeSchema.options
	};
	if (runtimeSchema instanceof zod.z.ZodDefault) return inferFieldType(runtimeSchema.def.innerType);
	if (runtimeSchema instanceof zod.z.ZodOptional) return inferFieldType(runtimeSchema.def.innerType);
	if (runtimeSchema instanceof zod.z.ZodNullable) return inferFieldType(runtimeSchema.def.innerType);
	if (runtimeSchema instanceof zod.z.ZodReadonly) return inferFieldType(runtimeSchema.def.innerType);
	return { type: "string" };
}
...
function loadConfigFile(path$2, options) {
	const resolvedPath = (0, path.resolve)(process.cwd(), path$2);
	let content;
	try {
		content = (0, fs.readFileSync)(resolvedPath, "utf-8");
	} catch (error) {
		if (isNodeError(error) && error.code === "ENOENT" && !options.required) return;
		if (isNodeError(error) && error.code === "ENOENT") throw new Error(`[konfuz] JSON config file not found: ${path$2}`);
		const reason = error instanceof Error ? error.message : String(error);
		throw new Error(`[konfuz] Could not read JSON config file "${path$2}": ${reason}`);
	}
	let data;
	try {
