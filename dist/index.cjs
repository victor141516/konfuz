...
function loadEnvFile(envPath) {
	const path$3 = envPath ?? (0, path.resolve)(process.cwd(), ".env");
	try {
		return (0, import_main.parse)((0, fs.readFileSync)(path$3, "utf-8"));
	} catch {
		return {};
	}
}
...
const require$2 = node_module.createRequire ? (0, node_module.createRequire)(require("url").pathToFileURL(__filename).href) : void 0;
const parser = new YargsParser({
	...
	require: (path$2) => {
		if (typeof require$2 !== "undefined") return require$2(path$2);
		else if (path$2.match(/\.json$/)) return JSON.parse((0, fs.readFileSync)(path$2, "utf8"));
...
var node_default = {
	...
	exists: (file) => {
		try {
			return (0, fs.statSync)(file).isFile();
		} catch (err) {
			return false;
		}
	}
...
var import_get_caller_file = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
const __dirname$1 = (0, url.fileURLToPath)(require("url").pathToFileURL(__filename).href);
...
var esm_default = {
	...
	getCallerFile: () => {
		...
		return callerFile.match(/^file:\/\//) ? (0, url.fileURLToPath)(callerFile) : callerFile;
	},
	stringWidth,
	y18n: y18n({
		directory: (0, path.resolve)(__dirname$1, "../../../locales"),
		updateFiles: false
	})
};
//#endregion
...
function parseCliArguments(info) {
	...
	for (const field of info.fields) {
		...
		if (field.enumValues) y = y.option(cliName, { choices: field.enumValues });
	}
	const parsed = y.argv;
	for (const field of info.fields) {
