...
function loadEnvFile(envPath) {
	const path$1 = envPath ?? (0, path.resolve)(process.cwd(), ".env");
	try {
		return (0, dotenv.parse)((0, fs.readFileSync)(path$1, "utf-8"));
	} catch {
		return {};
	}
}
...
function parseCliArguments(info) {
	const argv = (0, yargs_helpers.hideBin)(process.argv);
	const config = {};
